...
.text
main:
    # registers not to be used x0 - x4 , x10 - x17; reason to be explained later ***
    # registers that we can usde x5 - x9 , x18 - x31 ; reason to be explained later ***
...
loop1:
    bge x8 , x5, exit1  # check if i >= size go to exit 1
...
loop2:
    bge x8 , x5, exit2  # check if i >= size go to exit 2
...
loop3:
    bge x8 , x5, exit3  # check if i >= size go to exit 3
    #sum1 + arr1[i]
    #need &arr1[i]
    slli x18,x8, 2   # set x18 to i*4
    add x19, x18, x9 # add i*4 to the base address off arr1 and put it to x19
